var timecloud = angular.module('timecloud', ['sticky', 'ngRoute', "dndLists", 'ngSanitize', 'ui.bootstrap', 'employee', 'report', 'ui.bootstrap.datetimepicker', 'vxWamp']);
timecloud.config(function($routeProvider, $locationProvider, $wampProvider) {
    /*$momentProvider
        .asyncLoading(false)
...
timecloud.run(function($rootScope, $location, $http, myService, $wamp) {
    // Open
    $wamp.open();
    ...
    // Subscribe
    $wamp.subscribe('app.timecloud.heartbeat', onevent);

    $rootScope.$on('$routeChangeStart', function(event, next, current) {
        // Publish
        $wamp.publish('app.timecloud.route', ['Route Changed']);
...
/* current dashboard */
timecloud.controller('meterdashboardController', function($route, $scope, $rootScope, $http, $location, myService, holidayservice, $routeParams, $modal, $timeout, $parse, $wamp) { //header controller

    var idArry1 = [];
    var idArry = [];
    var weeklySubscribeStatus = false;
    ...
    /* dropdown days generation */
    function generateDropdownDays(start, end) {
        $scope.dateArray = [];
        while (start <= end) {
    ...
    function applyPrvBtnStyles(type) {
        if (!type) {
    ...
    function getDashboardData(deptList, data) {
        $http.get("/getdahboardData/" + data.start + "/" + data.end).success(function(dataDashs) {
            ...
            if (!weeklySubscribeStatus) {
                // subscribe dashboard
                weeklySubscribe(dataDash._id);
            } else
                console.log("Already subscribe to weekly");
            // create days array based on startDate and endDate
            generateDropdownDays(startDate, endDate);

            console.log("**weekly calFlag status : %s **", dataDash.calfalg);

            if (dataDash !== null) {
                var workHours = dataDash.workedHours.split(":");
                workHours = parseInt(workHours[0]);
                var moneySpent = dataDash.moneySpent;

                if (workHours == '0') $scope.actulPer = '00';
                else if (moneySpent == '0') $scope.actulPer = '00';
                else $scope.actulPer = Math.round(workHours * 100 / parseInt(moneySpent));

                if (dataDash.budgetedSales)
                    $scope.budgetedSale = dataDash.budgetedSales;

                applyPrvBtnStyles(dataDashs.prvsDatas);
                ...
                console.log("Work changed %s : %s", $scope.pData, $scope.lastPerData);
                ...
                if (deptList != 'false') {
                    console.log("** Subadmin view **");
                    if (dataDash.departments !== null) {
                        var newallocatedHours = 0,
                            newworkedHours = 0,
                            newmoneySpent = 0,
                            budgetedSalesTotal = 0,
                            budgetedWagesTotal = 0,
                            actualSalesTotal = 0,
                            result = [],
                            cnt = 0;
                        ...
                        deptList.forEach(function(key) {
                            ...
                            if (cnt == deptList.length) {
                                ...
                                if (newworkedHours != '0') dataDash.percentageUsed = (newworkedHours * 100) / newallocatedHours;
                                else dataDash.percentageUsed = 0;
    ...
    /* get daily dashboard */
    function getDateWiseData(deptList, dataDash, dataDashs) {
        if (dataDash !== null) {

            angular.forEach($scope.dateArray, function(dt) {
                // console.log(dt.day);
                if (dt.day <= moment().format('YYYY-MM-DD')) {
                    dt.activeDay = true;
                } else {
                    dt.activeDay = false;
                }
    ...
    function onEventDash(args) {
        // if (args[0].company == localStorage.getItem("userId")) {
        console.log("weekly Dashboard updated");
        getDashboardData($scope.adminDeptList, $scope.payPeriodData);
        // }
    }
    /* refresh current dashboard data */
    function onEventCurrentDash(args) {
        console.log("current date Dashboard updated");
        $http.post('/displayDashboardAcrodingNeed', $scope.currentDateDatas).success(function(dataDashs) {
            var dataDash = dataDashs.data;
            dataDash.weekEnd = $scope.currentDateDatas.end;
            dataDash.weekStart = $scope.currentDateDatas.start;
            getDateWiseData($scope.adminDeptList, dataDash, dataDashs);
        });
    }
    /* Weekly subscribe */
    function weeklySubscribe(id) {

        var channel = 'timecloud.dashboard.update.' + id;
        console.log('weeklySubscribe in DayController subscribe to ' + channel);
        $wamp.subscribe(channel, onEventDash);
        weeklySubscribeStatus = true;
        // console.log($wamp);
    }
    /* Current date subscribe */
    function currentDateSubscribe(id) {
        var channel = 'timecloud.dashboard.update.' + id;
        console.log('weeklySubscribe in DayController subscribe to ' + channel);
        $wamp.subscribe(channel, onEventCurrentDash);
    }
    ...
    /* on change date dashboard */
    $scope.dateWiseDashboard = function(date, start, end, companyId) {
        if (date.day !== "") {
            $scope.dynamicText = "day";
            //styles
            applyButtonStyles($scope.dynamicText);
            $("#showLoading").css("opacity", "0.2");
            $scope.showLoading = true;
            var myDay = date.day.split("-");
            $timeout.cancel($rootScope.timer1);
            $rootScope.timer1 = null;
            $location.path('/meterdashboard/' + start + '/' + end + '/' + myDay[2]);
        } else {
            $scope.dynamicText = "week";
            //styles
            applyButtonStyles($scope.dynamicText);
    ...
    /* till current date dashboard */
    $scope.currentDateDashboard = function(start, end, companyId) {
        $scope.dynamicText = "week";

        //styles
        applyButtonStyles("day");

        $scope.showLoading = true;
        $("#showLoading").css("opacity", "0.2");

        $scope.currentDateDatas = {
            start: start,
            end: end,
            companyId: companyId,
            currentDayCal: true
        ...
        $http.post('/displayDashboardAcrodingNeed', $scope.currentDateDatas).success(function(dataDashs) {
            if (dataDashs.data.companyId) {
                ...
                currentDateSubscribe(localStorage.getItem("userId"));
                // $wamp.subscribe('timecloud.meterdashboard.update', onEventMeter);
                // console.log('subscribe to timecloud.dashboard.update');
    ...
    $scope.getSubCategories = function(department) {
        var workHours = department.workedHours.split(":");
        workHours = parseInt(workHours[0]);
    ...
    /* next period dashboard */
    $scope.nextDashBoardDataFn = function(weekStart, weekEnd, companyId) {
        var startDate = moment(weekEnd).add(1, 'days').format("YYYY-MM-DD");
        var days = moment.duration(moment(weekEnd).diff(moment(weekStart))).asDays();
        var endDate = moment(weekEnd).add(days + 1, 'days').format("YYYY-MM-DD");
    ...
    $scope.previousDashBoardDataFn = function(weekStart, weekEnd, companyId) {
        $scope.dashboardData = [];
        var EndDate = moment(weekStart).subtract(1, 'days').format("YYYY-MM-DD");
        var days = moment.duration(moment(weekEnd).diff(moment(weekStart))).asDays();
        var startDate = moment(weekStart).subtract(days + 1, 'days').format("YYYY-MM-DD");
        // move to previous dashboard
        $location.path('/meterdashboard/' + startDate + '/' + EndDate);
    };
    ...
    /* update sub department budgeted sales data */
    ...
    //             subDepartmentId: id,
    //             weekEnd: $scope.to,
    //             weekStart: $scope.from,
    //             myDate: curDate,
    //             budgetedSale: budgetedSubDeptSale
    //         };
    //         // console.log(data);
    ...
    //     }
    // };
    /* update sub department actual sales data */
...
/* weekly dashboard */
timecloud.controller('meterDashboardNextControllers', function($route, $scope, $rootScope, $http, $location, myService, holidayservice, $routeParams, $modal, $timeout, $parse, $wamp) { //header controller
    var idArry1 = [];
    var idArry = [];
    var weeklySubscribeStatus = false;
    ...
    function getDashboardData(deptList, data) {
        $http.get("/getdahboardData/" + data.start + "/" + data.end).success(function(dataDashs) {
            ...
            if (!weeklySubscribeStatus) {
                ...
                console.log("Weekly dashboard already subscribe");
            }
...
/* daily dashboard */
timecloud.controller('meterdashboardDayController', function($route, $scope, $http, $location, myService, holidayservice, $routeParams, $modal, $timeout, $parse, $rootScope, $wamp) { //header controller
    var idArry1 = [];
    var idArry = [];
    ...
    /* daily dashboard */
    function getDateWiseData(deptList, myDatas) {
        // console.log(dataDash);
        $http.post('/getDateWiseRecord', myDatas).success(function(dataDashs) {
            if (dataDashs.data) {

                var dataDash = dataDashs.data;
                dataDash.weekEnd = myDatas.endDate;
                dataDash.weekStart = myDatas.startDate;

                // daily
                console.log(dataDash._id);
                dailySubscribe(dataDash._id);

                angular.forEach($scope.dateArray, function(dt) {
                    // console.log(dt.day);
                    if (dt.day === myDatas.date) {
                        dt.activeDay = true;
                    } else {
                        dt.activeDay = false;
                    }
                ...
                if (dataDash !== null) {
                    ...
                    console.log("**daily calFlag status : %s **", dataDash.calfalg);
                    changebutton(percentage, 'main');

                    // if ($scope.pData != $scope.lastPerData) {
                    console.log("Work percentage %s : %s", $scope.pData, $scope.lastPerData);
    ...
    /* upto current date dashboard */
    function getCurrentDateData(deptList, dataDash, dataDashs) {
        if (dataDash !== null) {
            angular.forEach($scope.dateArray, function(dt) {
                if (dt.day <= moment().format('YYYY-MM-DD'))
                    dt.activeDay = true;
    ...
    function onEventDash(args) {
        ...
        $http.post('/displayDashboardAcrodingNeed', $scope.currentDateDatas).success(function(dataDashs) {
            ...
            dataDash.weekEnd = $scope.currentDateDatas.end;
            dataDash.weekStart = $scope.currentDateDatas.start;
            getCurrentDateData($scope.adminDeptList, dataDash, dataDashs);
        });
    }
    /* refresh meter data */
    function onEventMeter(args) {
        getDateWiseData($scope.adminDeptList, $scope.currentDateDatas);
        console.log("Daily dashboard updated");
    }
    /* Weekly subscribe */
    function currentDateSubscribe() {
        var channel = 'timecloud.dashboard.update.' + localStorage.getItem("userId");
        console.log('weeklySubscribe in DayController subscribe to ' + channel);
        $wamp.subscribe(channel, onEventDash);
    }
    /* Daily subscribe */
    function dailySubscribe(id) {
        var channel = 'timecloud.meterdashboard.update.' + id;
        console.log('dailySubscribe in DayController  subscribe to ' + channel);
        $wamp.subscribe(channel, onEventMeter);
    }
    ...
    $scope.initDashboard = function(cal) {
        ...
        $scope.myDate = moment().date($route.current.params.day).format('YYYY-MM-DD');
        console.log($scope.myDate);
        console.log($route.current.params.startDate, $route.current.params.endDate);

        if ($scope.myDate >= $route.current.params.startDate && $scope.myDate <= $route.current.params.endDate) {
            $scope.currentDateDatas = {
                date: $scope.myDate,
                startDate: $route.current.params.startDate,
                endDate: $route.current.params.endDate
            };
            generateDropdownDays($scope.currentDateDatas.startDate, $scope.currentDateDatas.endDate);
            if ($route.current.params.startDate && $route.current.params.endDate && cal === true)
                $location.path('/meterdashboard');
            else {
                $http.get("/getPayperiod").success(function(periodicData) {
                    console.log($route.current.params.startDate);
                    console.log(periodicData.start);

                    if ($route.current.params.startDate == periodicData.start && $route.current.params.endDate == periodicData.end) {
                        $scope.showPeriod = true;
                    } else {
                        $scope.weekView = true;
                    }
    ...
    /* daily dashboard on change date */
    $scope.dateWiseDashboard = function(date, start, end, companyId) {
        if (date.day !== "") {
            applyButtonStyles($scope.dynamicText);
            $scope.dynamicText = "day";
    ...
    $scope.periodicDashboard = function(start, end, companyId) {
        ...
        if (moment().format('YYYY-MM-DD') >= startDate && moment().format('YYYY-MM-DD') <= endDate) {
            $location.path('/meterdashboard');
        } else {
            $location.path('/meterdashboard/' + startDate + '/' + endDate);
        }
    ...
    $scope.saveSubDeptActualSale = function(id, actualSales, index, curDate, allocatedHours) {
        ...
        if (parseInt(allocatedHours) > 0) {
            ...
            var data = {
                subDepartmentId: id,
                weekEnd: $route.current.params.endDate,
                weekStart: $route.current.params.startDate,
                myDate: curDate,
                actualSales: actualSales
            };
            console.log(data);
...
timecloud.controller('headerController', function($route, $rootScope, $scope, $http, $location, myService, holidayservice, $routeParams, $modal, $wamp,$sce) { //header controller
    ...
    $rootScope.$on('$routeChangeSuccess', function(e, current, pre) {
        ...
        if(current.$$route.controller){
            ...
            } else if(current.$$route.controller == "feedbackController"){
    ...
    // video modal
    $scope.openVideoModel = function(link) {
        console.log(link);
        var modalInstance = $modal.open({
        ...
        myService.setLink(link);
    };

    var VideoModalInstanceCtrl = function($scope, $modalInstance, myService) {
        console.log(myService.getLink());
        $scope.videoHeader = myService.getLink().name;
        var videoPath = 'https://editmytime.com/videos/'+ myService.getLink().path;
        console.log(videoPath);
        $scope.myLink = $sce.trustAsResourceUrl(videoPath);
        $scope.cancel = function() {
            $modalInstance.dismiss('cancel');
        };
    };
...
timecloud.controller('settingsController', function($route, $scope, $modal, $http, $location, holidayservice, myService) { //setting controller
    ...
    $http.get('/settings').success(function(data) {
        ...
        $scope.myPayPeriod = data.payPeriod;
        console.log("pp" + $scope.myPayPeriod);
        ...
        if (data.payPeriod) {
            var names = data.payPeriod + 'P';
            $('#weeklyP').removeClass('btn btn-primary active').addClass('btn btn-primary');
            $('#' + names).removeClass('btn btn-primary').addClass('btn btn-primary active');
        } else {
        ...
        if ($scope.settingForm.isdepartments) {
            if ($scope.settingForm.departmentname) {
                if ($scope.myPayPeriod) {
                    ...
                    error.department = "Please save the settings first";
                    $scope.message = error;
                }
            } else {
    ...
    $scope.openModel = function() {
        ...
        modalInstance.result.then(function(resultValue) {
            if (resultValue == 1) {
                ...
                error.added = "Department is in use, You can not delete it.";
                $scope.message = error;
            }
        }, function() {
    ...
    $scope.openModel1 = function() {
        ...
        modalInstance.result.then(function(resultValue) {
            if (resultValue == 1) {
                ...
                error.added = "Sub-Department is in use, You can not delete it.";
                $scope.message = error;
            }
        }, function() {});
    };
...
timecloud.controller('shiftController', function($scope, $http, $location, $timeout) { //shift controller
    ...
    $scope.formatTime = function(id, name) {
        ...
        if (val.length >= 3) {
            $scope[name] = val.substring(0, 2) + ':' + val.substring(2);
        } else {
            $scope[name] = val;
        }
        // if(val.length >= 5)
    ...
    $scope.formatTimeFinal = function(id, name) {
        if (id) {
            ...
            console.log(val);
            if (val.length == 2)
                $scope[name] = val.substring(0, 2) + ':00';
            else if (val.length == 1)
                $scope[name] = '0' + val + ':00';
            else if (val.length == 3)
                $scope[name] = val.substring(0, 2) + ':' + val.substring(2) + '0';
            // if(val.length >= 5)
            //     val = val.substring(0,5); 

            // $('#'+id).val(val);
            if (parseInt($scope[name].substring(0, 2)) > 23 || parseInt($scope[name].substring(3)) > 59) {
                var hr = $scope[name].substring(0, 2);
                var min = $scope[name].substring(3);
            ...
            console.log("final " + $scope[name]);
        }
    };

    $scope.otherTime = function(id) {
        var val = $('#' + id).val();
        val = val.replace(/[^0-9]/g, '');
        if (val.length >= 2)
            val = val.substring(0, 2) + ':' + val.substring(2);
            ...
            $http.get("/shift/edit/" + $routeParams.shiftId)
                .success(function(data) {
                    if (data) {
                        ...
                        var timezones = [];
                        // console.log(data.timeZones.length);
                        if (data.timeZones) {
                        ...
                        $scope.shift = data;
                        $scope.shift.timeZones = timezones;
                        console.log(data);
                        //      'name' : data.name,
                ...
                if (cnt == $scope.shift.timeZones.length) {
                    $http.get("/shift/edit/" + $routeParams.shiftId).success(function(data) {
                        ...
                        if ($scope.shift.name) {
                            ...
                            if (moment.utc(data.startTime).format('HH:mm') != $scope.startTime ||
                                moment.utc(data.finishTime).format('HH:mm') != $scope.finishTime ||
                                $scope.shift.startLimit != data.startLimit ||
                                $scope.shift.finishLimit != data.finishLimit ||
                                $scope.shift.breakAfter != data.breakAfter ||
                                $scope.shift.breakTime != data.breakTime ||
                                $scope.shift.overTime1 != data.overTime1 ||
                                $scope.shift.overTime2 != data.overTime2
                            ) {
...
timecloud.controller('shiftPatternController', function($scope, $http, $location, $routeParams) { // shift pattern list controller
    ...
    $scope.saveshiftOrder = function() {
        // console.log("dfgdf");
        $http.post('/saveShiftOrder', {
            shiftList: $scope.shiftList
        }).success(function(data) {
            if (data) {
                $http.get('/shiftData').success(function(data) {
                    if (data) {
...
timecloud.controller('editshiftPatternController', function($scope, $modal, $log, $http, $location, $routeParams, myService) { // edit shift pattern controller
    ...
    $http.get("/shiftpattern/edit/" + $routeParams.shiftPatternId)
        .success(function(data) {
            ...
            $scope.weekDayshow = 0;
            if (data.name) {
                $http.post("/checkassigneduser", {
                    shiftPattern: data.name
                }).success(function(data) {
                    if (data == 'false') {
                        $scope.deleteBtn = 1;
                    }
...
timecloud.filter('getDays', function() {
    return function(myDate) {
        var myDay = myDate.split("-");
        // console.log(myDay[0],myDay[1],myDay[2]);
        return myDay[2];
    };
});
